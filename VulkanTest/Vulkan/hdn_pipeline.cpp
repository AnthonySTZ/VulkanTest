...
namespace hdn {
	...
	PipelineConfigInfo HdnPipeline::defaultPipelineConfigInfo(uint32_t width, uint32_t height)
	{
		...
		// Config Viewport : Image drawn from our frag shader?
		...
		// Config Scissor : cutoff everything outside the range
		...
		// Config MultiSampling (MSAA, ...) : how the rasterizer handles the edges of geometry
		...
		// Config ColorBlending : how we combine colors in the frame buffer
	...
	void HdnPipeline::createGraphicsPipeline(const std::string& vertFilepath, const std::string& fragFilepath, const PipelineConfigInfo& configInfo)
	{
		...
		shaderStages[0].pName = "main"; // Name of the entry function in the shader
		...
		shaderStages[1].pName = "main"; // Name of the entry function in the shader
		...
		// Possible to enable multiple viewports
		...
		if (vkCreateGraphicsPipelines(hdnDevice.device(), VK_NULL_HANDLE, 1, &pipelineInfo, nullptr, &graphicsPipeline) != VK_SUCCESS) {
